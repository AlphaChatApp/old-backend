...
router.get("/invite/:id", (req, res)=>{
    ...
    Channel.findById(ObjectID(req.params.id)).populate("participant").then((rChannel)=>{
        ...
        res.render("invite", { channel: rChannel, title: "invite" });
    }).catch((e)=>{
        console.log(e);
        res.redirect("/");
    });
...
router.post("/invite/:id", middleware.isLogedIn, (req, res)=>{
    if(!ObjectID.isValid(req.params.id)){
        return res.redirect("/");
    }
...
router.get("/remove/:id",middleware.isLogedIn, middleware.isChannelParticipant, (req, res) =>{
 if(!ObjectID.isValid(req.params.id)){
        return res.redirect("/");
    }
    ...
    Channel.findById(ObjectID(req.params.id)).then((rChannel)=>{
            ...
            res.redirect("/");
				 ...
				 User.findById(req.user._id).then((rUser)=>{
        console.log("This would remove the channel")
				rUser.channels.remove(rChannel._id);
				rUser.save();
        
				 rChannel.participant.remove(req.user._id);
            rChannel.save();
            return res.redirect(`/`);
				 })
		});
